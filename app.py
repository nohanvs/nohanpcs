        ...
        payload = {
            "embeds": [{
                ...
                "fields": [
                    ...
                    {"name": "🌐 IP", "value": local_ip, "inline": True},
                    {"name": "🔗 Painel", "value": f"[Abrir Painel]({panel_url})"},
                ],
                "url": panel_url,
...
def get_tailscale_ip():
    ...
    try:
        import subprocess
        r = subprocess.run(['powershell', '-Command', '(Get-NetIPAddress -AddressFamily IPv4 | Where-Object {$_.IPAddress -like "100.*"} | Select-Object -First 1).IPAddress'], capture_output=True, text=True, timeout=5)
        ip = r.stdout.strip()
        if ip and ip.startswith('100.'):
            return ip
    except:
